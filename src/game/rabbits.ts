...
import { RABBIT_COUNT, isMountainCore, pickRabbitSpawns, riverDistance } from './worldLayout';
  ...
  update(delta: number, time: number, playerX: number, playerZ: number) {
    for (const r of this.rabbits) {
      ...
      if (r.state === 'flee' || r.state === 'hop') {
        ...
        const blocked = riverDistance(nx, nz) < 14 || isMountainCore(nx, nz) || tooCloseToTree;
        if (blocked) {
